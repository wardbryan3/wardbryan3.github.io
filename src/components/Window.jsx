...
export default function Window({ id, children, menubar }) {
  ...
  const frameSx = isMaximized
    ? {
        ...
        left: 0,
        width: '100%',
        height: `calc(100vh - ${topOffset + maxBottom}px)`,
        zIndex: win.zIndex,
        borderRadius: 0,
      }
    : {
        position: 'fixed',
        top: Math.max(topOffset, win.position.y),
        left: Math.max(0, win.position.x),
  ...
  return (
    ...
    >
      ...
      >
        {ICON_NAMES[win.icon] && (
          <Icon name={ICON_NAMES[win.icon]} size={14} style={{ marginRight: '0.4rem' }} />
        )}
        <span
          style={{
            position: 'absolute',
        ...
        >
          <button className="titlebar-btn" onClick={() => toggleMinimize(id)} style={btnStyle}>
            <svg viewBox="0 0 10 10" width="10" height="10">
              <rect x="1" y="4.5" width="8" height="1" fill="currentColor" />
            </svg>
          </button>
          <button className="titlebar-btn" onClick={() => toggleMaximize(id)} style={btnStyle}>
            {isMaximized ? (
              <svg viewBox="0 0 10 10" width="10" height="10">
                <rect
                  x="1"
      ...
      {menubar && (
        ...
        >
          {menubar.map((item) => (
            <span key={item} style={{ cursor: 'pointer', color: 'var(--text-muted)' }}>
              {item}
            </span>
          ))}
        </div>
      ...
      <div
        style={{ flex: 1, minHeight: 0, overflow: 'auto', background: 'var(--bg)', opacity: 0.9 }}
      >
        {children}
      </div>
